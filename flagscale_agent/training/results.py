...
"""Deterministic, dependency-free analysis of Megatron iteration logs.

Use one loss-reporting log per *attempt*, with log_interval=1. Do not concatenate
rank logs or resumed attempts. Run with ``python -m flagscale_agent.training.results
--request request.json``; the request has the same fields as ``analyze_results``.
