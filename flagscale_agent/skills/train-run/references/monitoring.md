...
If a wait/observation command is interrupted, check its returned job and owned PID, current-run logs and assigned-device state before waiting again. Do not replace an interrupted long sleep or `tail -f` with another blocking loop. A timed-out wait is not a stopped training job; stopping follows the ownership checks in the main skill.
