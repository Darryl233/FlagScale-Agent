...
`flagscale_agent.training_run` 执行 `flagscale train [model] -c <yaml> --test`，限制单次时长并返回测量摘要。
...
```bash
PYTHONUNBUFFERED=1 python -m flagscale_agent.training_run --request /workspace/configs/baseline-01.json
```
