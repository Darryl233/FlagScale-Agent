...
Use this method when configuration, logs, or a profile support a hypothesis about activation saving, replay, or offload. This may lower the activation peak or trade more saved memory for less replay. A cheap configuration comparison does not require full memory attribution first.
...
These are starting points, not limits on scope or order. When evidence points to activation saving or replay as an opportunity, investigate peak rank/stage and tensor lifetimes within budget. Explore better save boundaries, output-discard recomputation, replay scheduling, or activation offload. Implementation changes must remain within the task's authorization.
...
Keep layout, MBS/GBS, precision, and backend fixed for an ordinary comparison. A justified joint candidate is also allowed; specify each difference and dependency, and do not attribute the combined gain to one switch.
Reuse confirmed parameter mappings. Inspect consumers only if a field is rejected, rewritten, or lacks activation evidence. When switching modes, check whether existing `distribute_saved_activations` and staged overrides in the parent configuration remain valid.

## Additional checks

- **Ordinary configuration comparison:** Confirm effective parameters and target modules. Include first optimizer-state allocation and a complete update. Record available peak rank, stage, and memory measure. Reuse the main loop's general quality checks instead of reauditing every parameter group and checkpoint each time.
