...
Use this method when configuration, logs, or a profile support a hypothesis about compute granularity, dispatch overhead, pipeline bubbles, or activation capacity. A cheap MBS comparison does not require a full profile first.
...
These are starting points, not limits on scope or order. With evidence and budget, investigate actual GEMM/attention shapes, dispatch and communication calls per update, in-flight activations, and schedule bubbles to form a broader set of valid MBS or joint candidates. Confirm that extensions such as dynamic microbatching are integrated before using them; source changes must remain within the task's authorization.
Reuse confirmed field mappings. Inspect consumers only if a field is rejected, rewritten, or lacks evidence of taking effect.
...
- **Quality:** Reuse the main loop's common initial state, logical batch, and quality criteria. Variable-length masks, packing, or within-batch MoE statistics may break equivalence even at fixed GBS. When results differ, check loss weights, accumulation scaling, gradients, and updates as needed. Finite loss alone does not establish equivalence.
