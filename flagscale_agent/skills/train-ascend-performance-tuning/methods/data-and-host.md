...
Use this method when configuration, logs, or a profile support a hypothesis about data waits, CPU contention, or host dispatch overhead. A cheap configuration comparison does not require a full profile first.
...
These are starting points, not limits on scope or order. With evidence and budget, inspect dependencies across read, preprocessing, H2D, and execution; CPU hotspots; and implicit synchronization. Explore batched preprocessing or a transfer pipeline.
For a code candidate, name the target function and change; implement it when selected by the main loop and source edits are authorized. Asynchronous H2D must preserve source-buffer lifetime and consumer dependencies. Adding `non_blocking=True` alone does not establish overlap.
Reuse confirmed support and mappings; investigate only gaps relevant to the candidate. Do not reconvert valid data or replace it with mock data. Preserve layout, MBS/GBS, precision, model, and the task's required data, evaluation, and checkpoint behavior.
