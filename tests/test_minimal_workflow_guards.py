...
@pytest.mark.parametrize("command", [
    ...
    "ssh host 'flagscale train --help'",
    "cd /workspace && bash -c 'flagscale run -h'",
])
def test_help_does_not_force_training_monitor(command):
    ...
    assert guard.check_pre(GuardContext(tool_name="write_file")) is None
...
@pytest.mark.parametrize("command", [
    ...
    "cd /workspace && ssh host 'flagscale train --help;flagscale train model'",
    'flagscale train model -c "name;--help.yaml"',
])
def test_compound_query_does_not_hide_real_launch(command):
    ...
    # dev_0908 permits read-only diagnostics even while a launch is latched.
    verdict = guard.check_pre(GuardContext(tool_name="write_file"))
